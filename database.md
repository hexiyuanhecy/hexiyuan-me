...
### 3.5.1 Entry 父子关系（支持层级结构）
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| parent_id | UUID (FK → Entry) | 父条目ID（可空） |

**关系说明**：
- **工作经历 → 项目**：一个工作经历下可以包含多个项目，项目的 parent_id 指向工作经历的 entry_id
- **旅行 → 游记/地点**：一次旅行可以包含多个地点攻略，地点的 parent_id 指向旅行的 entry_id
- **其他类型**：支持任意层级关系，便于内容组织

**查询示例**：
```sql
-- 查询某工作经历下的所有项目
SELECT * FROM entry 
WHERE parent_id = '工作经历entry_id' AND type = 'project';

-- 查询某旅行下的所有地点
SELECT * FROM entry 
WHERE parent_id = '旅行entry_id' AND type = 'travel';
```
...
#### AIConfig（AI 配置表，V2 启用）
| 字段 | 类型 | 说明 |
| :--- | :--- | :--- |
| id | UUID (PK) | 配置 ID |
| provider | VARCHAR(50) | AI 服务商：`agnes` / `deepseek` |
| api_key | VARCHAR(255) | API 密钥（加密存储） |
| api_url | VARCHAR(500) | API 端点地址 |
| model | VARCHAR(100) | 默认模型名称 |
| enabled | BOOLEAN | 是否启用 |
| created_at | TIMESTAMP | 创建时间 |
| updated_at | TIMESTAMP | 更新时间 |

> MVP 阶段使用环境变量配置 AI 服务商，V2 阶段可迁移至此表实现动态配置。
