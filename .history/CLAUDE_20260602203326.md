...
2. 提供"批量粘贴"和"模板填写"两个选项卡。
3. 提交文本 → `/api/import` 调用 DeepSeek 分析，返回拆分后的条目数组。
4. 前端展示**条目卡片列表**，每条可：
- 修改 AI 提取的字段（标题、内容、日期、技术栈等）
- 勾选导入模块（工作经历类强制勾选 `resume`，其余预选可调）
- 上传多媒体附件（自动插入 Markdown 到正文）
- 删除（该条不导入）
- 手动拆分/合并条目
5. 确认后调用 `/api/import/confirm`，传入最终条目数组，后端事务写入数据库。
6. 完成后显示导入结果汇总，并提供跳转链接。

### 7.2 AI 分析提示词（`/api/import` 使用）
```
你是一个内容分类和拆分助手。分析以下文本，将其拆分为独立的条目（如多条工作经历、多个项目），并判断每条的类型，输出严格 JSON 数组。

类型选项：work_experience, project, knowledge_link, travel, food, daily, other
...
- knowledge_link: { title, url, category, tags[], description }
- travel: { destination, date, content_md, tags[] }
- food / daily / other: { title, date, content_md, tags[] }

规则：
- 若文本包含多段经历，务必拆分为数组，每条独立。
- 若用户按模板格式输入，优先提取对应字段。
- 脱敏：禁止提取真实姓名、具体薪资、身份证号。
- 时间字段若未提取到，留空。
...
{user_input}
---
请输出 JSON 数组，不含其他文字。格式示例：[{"type":"work_experience","fields":{...}},...]
```
### 7.3 入库逻辑
